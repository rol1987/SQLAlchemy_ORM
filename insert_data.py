...
# ident_publisher = input('Введите идентификатор издателя: ')

# q = session.query(Publisher).join(Book.publisher).filter(Book.id_publisher == ident_publisher)
# q = session.query(Publisher).join(Book.publisher).filter(Book.id_publisher == ident_publisher)
# q1 = session.query(Book).join(Stock.book).filter(Book.id_publisher == ident_publisher)
# q2 = session.query(Shop).join(Stock.shop).filter(Shop.id == q1.)
# print(q)
